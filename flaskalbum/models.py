...
from flaskalbum import USER_INFO_TABLE, mysql, bcrypt, app
import jwt
...
class User:
    ...
    def register_user(self, name, email, username, password):
        ...
        cursor.execute(f"SELECT * FROM {USER_INFO_TABLE} WHERE username = %s", (username,))
        user_with_username = cursor.fetchone()
        cursor.execute(f"SELECT * FROM {USER_INFO_TABLE} WHERE email = %s", (email,))
        ...
        if user_with_username:
            ...
            cursor.execute(f"INSERT INTO {USER_INFO_TABLE} (name, email, username, password) VALUES (%s, %s, %s, %s)",
    ...
    def authenticate_user(self, username, password):
        cursor = mysql.connection.cursor()
        cursor.execute(f"SELECT username, password FROM {USER_INFO_TABLE} WHERE username = %s OR email = %s", (username, username))
    ...
    def update_password(email, password):
        hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
        cursor = mysql.connection.cursor()
        cursor.execute(f"UPDATE {USER_INFO_TABLE} SET password = %s WHERE email = %s", (hashed_password, email))
        mysql.connection.commit()
        cursor.close()
    ...
    def user_details(self, username):
        cursor = mysql.connection.cursor()
        cursor.execute(f"SELECT name, email FROM {USER_INFO_TABLE} WHERE username = %s", (username,))
        user_data = cursor.fetchone()
        cursor.close()
        return user_data
    ...
    def update_info(self, username, update_username, name, email):
        cursor = mysql.connection.cursor()
        cursor.execute(f"UPDATE {USER_INFO_TABLE} SET username=%s, name=%s, email=%s WHERE username=%s", (update_username, name, email, username))
        mysql.connection.commit()
        cursor.close()
    ...
    def delete_acc(self, username):
        cursor = mysql.connection.cursor()
        cursor.execute(f"DELETE FROM {USER_INFO_TABLE} WHERE username=%s", (username,))
        mysql.connection.commit()
        cursor.close()
